...
class SpatioTemporalLSTMCell2(nn.Module):
    def __init__(self, in_channel, hidden_dim, configs):
        super(SpatioTemporalLSTMCell2, self).__init__()
    ...
    def forward(self, x_t, h_t, c_t, m_t):
        ...
        return h_new, c_new, m_new, delta_c, delta_m  # 也就是这里不一样


if __name__ == '__main__':
    from configs.predrnnv2_config import configs

    parse = configs()
    configs = parse.parse_args()
    hidden = []

    model = SpatioTemporalLSTMCell2(configs.img_channel, configs.num_hidden, configs).cuda()
    print("Model size: {:.5f}M".format(sum(p.numel() for p in model.parameters()) / 1000000.0))

    a = torch.randn((configs.batch_size, configs.img_channel, configs.img_width // configs.patch_size // configs.sr_size,
    ...
    c = m = h
    h, c, m, deltac, deltam = model(a, h, c, m)
    print(h.shape)
